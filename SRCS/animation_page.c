...
void	slide_page(t_data *data, int side)
{
	if (data->slide.animation)
	{
		...
		data->slide.i = 0;
		if (side == LEFT)
			data->slide.start = data->slide.pos - WIN;
		else
		{
			data->slide.start = data->slide.pos + WIN; // TODO fix pb image
			printf("PB %d\n", data->slide.start);
		}
	}
	else
...
//	}
	...
	data->slide.pos = get_value(data->slide.start, 0, data->slide.i++) * WIN / \
1000;
	if (data->slide.side == LEFT)
		x_from = data->slide.pos + WIN;
	else
	{
//		printf("%d | %d - %d\n", data->slide.pos, data->slide.start, 0);
	...
	if (data->in_menu)
		set_names_fractal(data, data->img.img, data->slide.pos);
	else
