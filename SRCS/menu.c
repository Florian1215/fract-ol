...
t_pos pos)
{
	...
	hover = 1;
	if (data->hover[pos].is_on)
		hover = data->hover[pos].zoom.value;
	co.x = ((i.x - frac->menu.start.x) - HWIN / 4 + (frac->offset_menu.x * \
WIN / 1000)) / QWIN * (frac->size_zoom * hover);
	co.y = ((i.y - frac->menu.start.y) - HWIN / 4 + (frac->offset_menu.y * \
WIN / 1000)) / QWIN * (frac->size_zoom * hover);
	return (co);
}
